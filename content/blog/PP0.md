...
共享公共部分数据，差异化数据独有。
### 举个例子：树的模型
文中拿渲染相关的来举例子，但是一般引擎都有对应的优化方法来。通过这个可以引申到代码方面的享元。
如果要在游戏场景里布置一个森林，那么树的模型就要有网格纹理位置等信息。如果每个树（相同的建模）都单独存放网格信息的话，gpu要读太多数据来。
- 我们可以把共有的部分提取出来：网格信息，纹理
- 保留独有的部分：位置，大小
这时候，gpu渲染时，只需要读取一次网格信息，因为每棵树都是相同的网格就不用重复发给gpu读了。然后各个树独有的部分再单独发给gpu，这可以节约一部分的性能。
这里我们如果要实现差异化的纹理怎么办？毕竟我们已经把它作为共有的部分了。我们可以把几种不同纹理塞在一张大图里面，通过uv偏移实现差异化的纹理效果。
+>
这里说句题外话，通过一张张拆分的图来做动画是可以的，但是一般的游戏教程里都是用一张图集，因为这可以减少gpu读取的次数，原先需要读好几张图，现在只需要读一次虽然这张图有点大。
<+
----
2026.5.31
----
